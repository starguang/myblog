...
然而我没想到的是关键帧keyframes也会被css modules加上哈希（css modules加哈希是为了不同页面有相同的类名的时候样式不发生冲突），所以当然就无法找到相应的关键帧的名字了，经过一番努力（主要是Google和github），最终在[这里](https://github.com/webpack-contrib/css-loader/issues/243)找到了想要的答案。
